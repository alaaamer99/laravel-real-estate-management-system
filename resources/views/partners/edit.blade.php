...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h3 mb-0">تعديل الطرف: {{ $partner->name }}</h1>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="mb-2">
                    ...
                    <span class="text-muted">{{ $partner->created_at->format('Y-m-d H:i') }}</span>
                </div>
                <div class="mb-2">
                    <strong>آخر تحديث:</strong>
                    <span class="text-muted">{{ $partner->updated_at->format('Y-m-d H:i') }}</span>
                </div>
                <div class="mb-2">
                    <strong>عدد العقارات المرتبطة:</strong>
                    <span class="badge bg-info">{{ $partner->properties()->count() }}</span>
                </div>
                
                @if($partner->properties()->count() > 0)
                <div class="alert alert-warning mt-3">
                    <i class="bi bi-exclamation-triangle"></i>
                    تنبيه: يوجد عقارات مرتبطة بهذا الطرف. تأكد من صحة التعديل.
                </div>
                @endif
            </div>
        </div>
